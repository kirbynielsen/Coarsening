...
(function(){
  ...
  function boot(){ if(window["__pmBooted_pm_choice_root"])return; var root=document.getElementById("pm-choice-root"); if(!root){return setTimeout(boot,40);} window["__pmBooted_pm_choice_root"]=true; var ph=root.querySelectorAll(".pm-loading"); for(var i=0;i<ph.length;i++){ph[i].parentNode.removeChild(ph[i]);} var st=document.createElement("style"); st.textContent=CSS; (document.head||document.documentElement).appendChild(st); root.insertAdjacentHTML("beforeend",HTML); run(); }
  function run(){
