...
(function() {
  ...
  function renderHistory() {
    ...
    if (history.length === 0) {
      list.innerHTML = '<div class="history-empty">No results yet — submit a search above and click "Show result" to add it here.</div>';
      countEl.textContent = '';
      return;
    }
    ...
    list.innerHTML = history.map((h, displayIdx) => {
      ...
      return '<div class="history-item ' + (isLatest ? 'latest' : '') + '" draggable="true" data-idx="' + displayIdx + '">'
        ...
        +   '<span class="hist-among">Among candidates that are/have</span> ' + condHTML
