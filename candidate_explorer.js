...
(function(){
  ...
  function boot(){ if(window["__pmBooted_pm_root"])return; var root=document.getElementById("pm-root"); if(!root){return setTimeout(boot,40);} window["__pmBooted_pm_root"]=true; var st=document.createElement("style"); st.textContent=CSS; (document.head||document.documentElement).appendChild(st); root.insertAdjacentHTML("beforeend",HTML); run(); }
  function run(){
...
(function() {
  ...
  function ensureLightStyles() {
    ...
    extra.forEach(v => {
      ...
      css += '.mini-chip.' + k + '.state-on{background:' + stroke + ';border-color:' + text + ';color:#fff;}';
      ...
      css += '.mini-chip.' + k + '.state-off{background:' + pale + ';border-color:' + glow + ';color:' + stroke + ';}';
    });
