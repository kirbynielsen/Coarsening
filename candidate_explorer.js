/* Candidate outcome explorer — external build for Qualtrics.
   Put a <div id="pm-root"></div> in your question, then load this file with
   <script src="...candidate_explorer.js"></script>. Runs in the page, so
   window.PM_CONFIG and Qualtrics.SurveyEngine.setEmbeddedData both work.
   Optional config: add <div id="pm-config" style="display:none">{...JSON...}</div>. */
(function(){
...
(function() {
  ...
  function renderDatabase() {
    ...
    const totalOk = database.filter(m => m.success).length;
    meta.textContent = N_CAND.toLocaleString() + ' candidates · ' + totalOk.toLocaleString() + ' ' + CFG.successWord + ' (' + (totalOk/N_CAND*100).toFixed(1) + '%)';
  ...
  function syncData() {
    ...
    try {
      if (window.Qualtrics && Qualtrics.SurveyEngine && typeof Qualtrics.SurveyEngine.setEmbeddedData === 'function') {
        Qualtrics.SurveyEngine.setEmbeddedData('probabilityMachineData', json);
      }
    } catch (_) {}
    return json;
  }
  ...
  if (exportBtn) exportBtn.addEventListener('click', () => {
    ...
    a.download = 'candidate-explorer-' + PARTICIPANT_ID + '.json';
    a.click();
