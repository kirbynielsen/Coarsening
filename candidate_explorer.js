...
(function() {
  ...
  let queryCounter = 0;
  const NOTES = { text: '', edits: 0, lastEditedMs: null };   // optional free-text notes
  ...
  function syncData() {
    ...
    try {
      ...
      var setED = (qe && (qe.setJSEmbeddedData || qe.setEmbeddedData)); if (typeof setED === 'function') setED.call(qe, 'label_order', LABEL_ORDER.join(','));
    } catch (_) {}
  ...
  function valueChip(v, state) {
    if (v.kind === 'outcome') {
      return '<span class="mini-chip kind-sound state-' + state + '">' + (state === 'on' ? OUTCOME_SVG : '') + stateLabel(v, state) + '</span>';
    }
  ...
  function renderBlank(blankEl, listArr, zoneKey, emptyClass) {
    ...
    let html = zoneKey === 'cond' ? '<span class="qb-with">with</span>' : '';
    groups.forEach((g, gi) => {
      if (gi > 0) {
  ...
  function renderHistory() {
    ...
    list.innerHTML = history.map((h, displayIdx) => {
      ...
      const condHTML = h.conditions.length === 0
        ? ''
        : '<span class="hist-among">with</span> ' + exprHTML(h.conditions);
      ...
      return '<div class="history-item ' + (isLatest ? 'latest' : '') + '" draggable="true" data-idx="' + displayIdx + '">'
        ...
        +   '<span class="hist-among">Among all candidates</span> ' + condHTML
  ...
  function quizSetup() {
    ...
    const np = rootEl.querySelector('#notes-pad'); if (np) np.remove();
    const ins = rootEl.querySelector('.instructions'); if (ins) { ins.style.marginTop = '1.75rem'; } if (ins) ins.innerHTML = 'Build the request by <strong>dragging chips</strong> from the tray into the blanks \u2014 or use the <strong>+</strong> button in each blank. Click the <strong>and / or</strong> pill to change how chips are combined. Then click <strong>Submit request</strong>.';
    document.getElementById('submit-btn').textContent = 'Submit request';
  }
  (function wireNotes() {
    const ta = document.getElementById('pm-notes');
    if (!ta) return;
    let timer = null;
    ta.addEventListener('input', () => {
      NOTES.text = ta.value; NOTES.edits++; NOTES.lastEditedMs = Date.now() - SESSION_START;
      clearTimeout(timer); timer = setTimeout(() => logEvent('notes_edit', { chars: NOTES.text.length }), 800);   // sync (debounced), no text in the event stream
    });
    ta.addEventListener('blur', () => syncData());
  })();
